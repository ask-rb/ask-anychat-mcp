# Versioning — ask-anychat-mcp

This file is the repository's **canonical versioning and release policy** for
`ask-anychat-mcp`. If any other document (README, commit messages, notes)
...
numbers (`0.2.0` → `0.2.5` is wrong), never skip a minor, and never reuse
a version that has already been published.

## Release tooling

- All `ask-*` gems and `yamine` release exclusively through **gemchain**
  (`gemchain update ask-anychat-mcp <ver>` from the workspace root). Never
...
- **Everything must agree.** The published RubyGems version, the version in
  `lib/ask/anychat/mcp/version.rb`, the `vX.Y.Z` git tag, and the source at
...
- **Dependency releases use the gemchain cascade.** When a dependency of
  `ask-anychat-mcp` (e.g. `ask-mcp`, `ask-anychat`) ships, gemchain's
...
9. **Verify** — RubyGems, `lib/ask/anychat/mcp/version.rb`, the tag, and
   the pushed source all report `X.Y.Z`.

## Examples

- Bug fix in `0.2.0` → release `0.2.1` (PATCH, one step).
- New tool in `0.2.1` → release `0.3.0` (MINOR; a pre-1.0 breaking change
  also goes here).
- Never `0.2.0` → `0.2.5`; the patch number moves by exactly one per
  release.
- Dependency cascade: `ask-mcp` ships `0.6.0`; from the workspace root run
  `gemchain update ask-mcp 0.6.0` — gemchain rewrites this gem's constraint,
  runs its tests, and releases the next `ask-anychat-mcp` patch. No hand
  edits.
